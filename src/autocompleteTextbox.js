...
function AutocompleteTextbox() {
  ...
  const queryGPT = async (input) => {
    ...
    const prompt = "Imagine you are an autocomplete feature tailored to provide suggestions for a restaurant review in the style of Yelp. Your goal is to assist the user in completing their sentences by offering contextually relevant endings that highlight their experience with their restaurant visit. As you generate suggestions, lean towards choices that emphasize the high quality of the food, the service, and the pleasure of the experience. Encourage the user to express their positive experiences. An autocomplete feature finishes the sentece without suggesting what came before, so ONLY FINISH THE SENTENCE, without repeating the part I am about to tell you. How would you COMPLETE the following sentence? (DO NOT REPEAT THE BEGINNING)"
