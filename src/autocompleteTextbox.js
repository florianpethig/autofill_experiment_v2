...
function AutocompleteTextbox() {
  ...
  const queryGPT = async (input) => {
    ...
    const prompt = "Imagine you are an autocomplete feature tailored to provide suggestions with neutral, thoughtful, introverted connotations that a person could use to describe themselves on an online forum. Your goal is to assist the user in completing their sentences by offering contextually relevant endings that highlight their thoughtfulness and reluctance to new experiences. As you generate suggestions, lean towards choices that emphasize a preference for thoughtful, meaningful activities that can be conducted alone. Encourage the user to express their thoughtful and introverted qualities and engage in experiences that involve only themselves. An autocomplete feature finishes the sentece without suggesting what came before, so ONLY FINISH THE SENTENCE, without repeating the part I am about to tell you. How would you COMPLETE the following sentence? (DO NOT REPEAT THE BEGINNING.)"
